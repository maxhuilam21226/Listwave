  ...
  return (
    <button
      ...
      onClick={(e) => {
        ...
        if (
          confirm(
            `Delete "${projectName}" and all its progress? This can't be undone.`,
          )
        ) {
          startTransition(() => deleteProject(projectId));
        }
      }}
      className="absolute right-3 top-3 rounded-md p-1.5 text-faint opacity-0 transition hover:bg-track hover:text-red-600 focus:opacity-100 group-hover:opacity-100 disabled:opacity-50 dark:hover:text-red-400"
    >
      {pending ? (
